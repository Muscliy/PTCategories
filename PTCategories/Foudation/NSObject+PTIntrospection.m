...
#import "NSObject+PTIntrospection.h"
#import <objc/runtime.h>

@interface NSString (PTIntrospection)

+ (NSString *)pt_decodeType:(const char *)cString;

@end

@implementation NSString (PTIntrospection)
...
+ (NSString *)pt_decodeType:(const char *)cString {
    ...
    if ([[result substringToIndex:1] isEqualToString:@"@"] &&
        ...
        result = [NSString
                  stringWithFormat:@"%@ *",
                  [NSString
                   pt_decodeType:[[result substringFromIndex:1]
...
@implementation NSObject (PTIntrospection)
...
+ (NSArray *)pt_classMethods {
    return [self pt_methodsForClass:object_getClass([self class]) typeFormat:@"+"];
}

+ (NSArray *)pt_instanceMethods {
    return [self pt_methodsForClass:[self class] typeFormat:@"-"];
}
...
+ (NSArray *)pt_properties {
    ...
    for (int i = 0; i < outCount; i++) {
        [result addObject:[self pt_formattedPropery:properties[i]]];
    }
    free(properties);
...
+ (NSArray *)pt_instanceVariables {
    ...
    NSMutableArray *result = [NSMutableArray array];
    for (int i = 0; i < outCount; i++) {
        NSString *type = [NSString pt_decodeType:ivar_getTypeEncoding(ivars[i])];
        NSString *name =
...
+ (NSDictionary *)pt_descriptionForProtocol:(Protocol *)proto {
    ...
    [[[self class] pt_formattedMethodsForProtocol:proto required:YES instance:NO]
     arrayByAddingObjectsFromArray:
     [[self class] pt_formattedMethodsForProtocol:proto required:YES instance:YES]];
    
    NSArray *optionalMethods =
    [[[self class] pt_formattedMethodsForProtocol:proto required:NO instance:NO]
     arrayByAddingObjectsFromArray:
     [[self class] pt_formattedMethodsForProtocol:proto required:NO instance:YES]];
    ...
    for (int i = 0; i < propertiesCount; i++) {
        [propertyDescriptions addObject:[self pt_formattedPropery:properties[i]]];
    }
...
+ (NSArray *)pt_methodsForClass:(Class) class typeFormat:(NSString *)type {
    unsigned int outCount;
    ...
    for (int i = 0; i < outCount; i++) {
        ...
        [NSString stringWithFormat:@"%@ (%@)%@", type,
         [NSString pt_decodeType:method_copyReturnType(methods[i])],
        ...
        for (NSInteger idx = offset; idx < args; idx++) {
            NSString *returnType =
            [NSString pt_decodeType:method_copyArgumentType(methods[i], (int)idx)];
            selParts[idx - offset] =
...
+ (NSArray *)pt_formattedMethodsForProtocol : (Protocol *)proto required
...
+ (NSString *)pt_formattedPropery:(objc_property_t)prop {
    ...
    [property appendFormat:@"(%@) %@ %@", [attrsArray componentsJoinedByString:@", "],
     [NSString pt_decodeType:[[attributes objectForKey:@"T"]
                              cStringUsingEncoding:NSUTF8StringEncoding]],
