...
@implementation UIDevice (PTExtents)
...
- (BOOL) pt_isSimulator {
    ...
    dispatch_once(&onceToken, ^{
        simulator = NSNotFound != [[self model] rangeOfString:@"Simulator"].location;
    });
    return simulator;
}

- (BOOL) pt_isJailbroken {
    if ([self pt_isSimulator]) return NO;
    ...
    NSString *path = [NSString stringWithFormat:@"/private/%@", [NSString pt_UUID]];
...
- (int64_t) pt_diskSpaceUsed {
    int64_t total = self.pt_diskSpace;
    int64_t free = self.pt_diskSpaceFree;
